<x-app-layout>
    ...
    <div class="mb-xl filters-card">
        <div class="filters-desktop"
            style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: var(--spacing-md); align-items: center;">
            <div class="form-group mb-0" style="position: relative;">
                <div
                    style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: var(--gray-400); font-size: 1.125rem;">
                    <i class="fas fa-search"></i>
                </div>
                <input type="text" id="searchInput" class="form-input" placeholder="Buscar cursos..."
                    ...
                    <div style="position: relative; flex: 1;">
                        ...
                        <select id="profesorFilter" class="form-select"
                            ...
                            @foreach($cursos->unique('profesor_id')->filter(fn($c) => $c->profesor) as $curso)
                                <option value="{{ $curso->profesor->id }}">{{ $curso->profesor->nombre }}
        ...
        <div class="filters-modal-content" onclick="event.stopPropagation()">
            ...
            <div class="filters-modal-body">
                <div class="form-group">
                    ...
                    <input type="text" id="searchInputMobile" class="form-input" placeholder="Buscar cursos..."
            ...
            <div class="filters-modal-footer">
                ...
                <button onclick="applyFilters()"
                    style="flex: 1; background: #84cc16; color: white; border: none; border-radius: var(--radius-md); font-weight: 600; padding: 0.625rem;">
                    Aplicar
                </button>
            </div>
        </div>
        ...
        <div class="mobile-table-container"
            ...
            @forelse($cursos as $curso)
                <div class="curso-item mobile-table-row"
                    ...
                    data-search="{{ strtolower($curso->nombre . ' ' . $curso->nivel) }}" data-nivel="{{ $curso->nivel }}"
                    ...
                    <div style="display: flex; align-items: center; gap: var(--spacing-sm);">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div
                                style="font-size: 0.75rem; color: var(--gray-600); display: flex; align-items: center; gap: 6px; flex-wrap: wrap; margin-top: 2px;">
                                @if($curso->profesor)
    ...
    <div class="table-container">
        <table class="table">
            ...
            <tbody>
                @forelse($cursos as $curso)
                    <tr class="curso-item" style="cursor: pointer;"
                        onclick="window.location='{{ route('courses.show', $curso->id) }}'"
                        data-search="{{ strtolower($curso->nombre . ' ' . $curso->nivel) }}"
                        data-nivel="{{ $curso->nivel }}" data-profesor="{{ $curso->profesor_id ?? '' }}">
                        <td>
                        ...
                        <td>
                            <span
                                style="display: inline-block; padding: 0.25rem 0.625rem; font-size: 0.6875rem; font-weight: 700; letter-spacing: 0.05em; color: #84cc16; background: rgba(132, 204, 22, 0.1); border-radius: 9999px;">{{ strtoupper($curso->nivel) }}</span>
                        </td>
                        <td style="color: var(--gray-600);">
                            @if($curso->profesor)
                                <div style="font-weight: 500;">{{ $curso->profesor->nombre }} {{ $curso->profesor->apellido }}
                                </div>
                            @else
                        ...
                        <td>
                            <div style="display: flex; align-items: center; gap: var(--spacing-xs);">
                                ...
                                <span
                                    style="font-weight: 600; color: var(--gray-900);">{{ $curso->estudiantes_count ?? 0 }}</span>
                            </div>
                        </td>
                        <td>
                            <div style="display: flex; align-items: center; gap: var(--spacing-xs);">
                                ...
                                <span
                                    style="font-weight: 600; color: var(--gray-900);">{{ $curso->asignaturas_count ?? 0 }}</span>
                            </div>
                        </td>
                        @if(auth()->user()->isAdmin())
                        <td>
            ...
            <form action="{{ route('courses.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="filters-modal-body">
                    <p style="color: var(--gray-600); margin-bottom: var(--spacing-md); font-size: 0.9375rem;">
                        Sube un archivo CSV con tus cursos. Asegúrate de incluir las columnas en este orden (con
                        encabezado):
                    </p>
                    <ul
